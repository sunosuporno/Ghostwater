...
export async function fetchMarginManagerStates(params: {
  ...
  const raw = await get<MarginManagerState[] | unknown>('/margin_manager_states', params);
  return Array.isArray(raw) ? raw : [];
}
...
export async function fetchLoanBorrowed(params: {
  ...
  const raw = await get<LoanBorrowedEvent[] | unknown>('/loan_borrowed', params);
  return Array.isArray(raw) ? raw : [];
}
...
export function fromPythRaw(raw: string | number, decimals: number = PythDecimals): string {
